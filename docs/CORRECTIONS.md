...
`match` は式として使えます。各アームは `パターン => 式` で、必要なら **ガード**
（`パターン if 条件 => 式`）を付けられます。アーム区切りは改行（または `,`）。
...
| パターン | 例 | 説明 |
| --- | --- | --- |
| ワイルドカード | `_` | 何にでも一致（フィールド位置でも可） |
| 整数リテラル | `1`, `42` | 値が等しいとき一致 |
| 論理リテラル | `true`, `false` | |
| 文字列リテラル | `"a"` | |
| 範囲 | `1..=10` / `1..10` | 閉区間 / 半開区間（整数） |
| 結合 / OR | `1 \| 2 \| 3` | いずれかに一致 |
| 限定バリアント | `Shape.Circle(r)` | 列挙型のバリアントとフィールド |
| 裸バリアント | `Ok` | フィールドを持たないバリアント |
| 入れ子 | `Outer.Box(Inner.Num(n))` | パターンを再帰的にネスト |
| 束縛 | `x`, `Shape.Circle(r)` | 一致した値を名前に束縛 |
| グループ化 | `(a \| b)` | 優先順位の制御 |

#### 束縛（バインド）と大文字小文字

パターン内の**小文字で始まる bare 識別子**は**束縛**（新しい変数を導入）です。
整合した値がその名前で使えます。バリアントフィールド位置の bare 識別子も束縛
になります（例: `Shape.Circle(r)` の `r`）。

```ngs
enum Shape { Circle(i32) }
fn r(s: Shape) -> i32 {
    return match s {
        Shape.Circle(radius) => radius,   // radius に束縛
        _ => 0,
    }
}
```

- **大文字で始まる** bare 識別子は**裸バリアント**として扱われます（例: `Ok`）。
- トップレベルの bare 束縛（例: `x => "D"`）は**網羅的**とみなされ、`_` 不要です。
- 腕のパターンが `_`（またはトップレベルの束縛）を含めば、そのアームは網羅に寄与し、
  それ以降に追加の `_` は不要です。
...
ガードの条件式は `bool` でなければなりません。

#### 網羅性チェック

`match` は網羅性をチェックします。
- **列挙型**をスキャンする場合、取り得る**すべてのバリアント**をカバーする必要があり、
  その場合は末尾の `_`（またはオーバーオールの束縛）を省略できます。カバーされていない
  バリアントがあるとエラーになります（例: `Color.Red` と `Color.Green` のみで
  `Blue` を省略すると `missing variant(s) Blue`）。
- **スカラー**（`i32`, `bool`, `string` など）は `_`（またはオーバーオールの束縛）が必要です。

#### 値としてのバリアント生成

`HttpStatus.Ok` を**値として**生成するときは**括弧**が必要です（`HttpStatus.Ok()`）。
この括弧の規則はパターンには影響しません（パターンの裸バリアント `Ok` は括弧不要）。
...
### 5.5 トレイトのスコープ（拡張要求が来る前に線を引く）

`trait` は**型クラス的・多相的制約**として最小限サポートします。スコープを先に決めて
おき、将来の過剰な拡張要求を断れるようにします。

- **関連型（associated type）は持たない。**
- **演算子オーバーロード（`+` 等の再定義）は行わない。**
- **型パラメータ 1 つにつき境界は 1 つまで**（例: `fn f<T: Foo>(...)`。複数境界は不可）。
- トレイト自体の多相的ジェネリック実装（trait のためのジェネリクス）はこのスコープ外。まずは
  具象的な実装 + `impl` によるメソッド確認を優先する。

#### 3.4 構文の決定（`and/or/not`・括弧なし enum は不採用）

マスタープランの 3.4 提案（論理 `and`/`or`/`not` への統一、括弧なし enum 値生成）は
**採用しません**。理由: 既に確定済みの決定群（`&&`/`||`/`!`、`Color.Red()` の括弧）と
実コンパイラの挙動に合致しており、後方互換を壊すため。

- 論理演算子は **`&&` / `||` / `!`**（`and`/`or`/`not` は演算子としては**実装されていない**。
  単なる識別子として扱われるだけで、予約語でも無い）。
- ポインタのアドレス取得は **`&`（単一）**。`&&` と明確に区別。
- enum 値生成は**常に括弧**（`Color.Red()`）。データを持つバリアントも括弧。
- 文字列型は **`string`**（`str` ではない）を正式な名前とする。
