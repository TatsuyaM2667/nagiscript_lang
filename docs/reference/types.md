...
```ngs
val a: i64 = 100
val b: i32 = a  // エラー: i64 → i32 は安全な縮小ではない。明示的キャスト `as i32` が必要

val x: i32 = 42
val y: f64 = x  // OK: i32 から f64 への拡張（情報が失われない場合のみ暗黙可）
```

> **安全性方針**: 数値の**拡張**（i32 → i64 等、情報が失われない）のみ暗黙に許可します。
> **縮小変換**（i64 → i32 等、オーバーフローでデータが失われる可能性があるもの）は常に明示的な `as` を要求します。
